...
def worker_v1(timeout, start_time, shared_best, lock):
    number = 3
    jump_limit = 100
...
def find_max_prime_parallel_v1(timeout, workers):
    ...
    with multiprocessing.Manager() as manager:
        ...
        for _ in range(workers):
            p = multiprocessing.Process( target=worker_v1, args=(timeout, start_time, shared_best, lock) )
            processes.append(p)
            p.start()
...
def worker_interleaving(worker_id, workers, timeout, start_time, shared_best, lock):
...
def find_max_prime_parallel_v2(timeout, workers):
    ...
    with multiprocessing.Manager() as manager:
        ...
        for i in range(workers):
            p = multiprocessing.Process( target=worker_interleaving, args=(i, workers, timeout, start_time, shared_best, lock) )
            processes.append(p)
            p.start()
...
def worker_chunks(timeout, start_time, chunk_queue, shared_best, lock):
    while (time.time() - start_time) < timeout:
        try:
...
def find_max_prime_parallel_v3(timeout, workers):
    start_time = time.time()
    chunk_size = chunk_size = workers * 100_000
    ...
    with multiprocessing.Manager() as manager:
        ...
        for _ in range(workers):
            p = multiprocessing.Process( target=worker_chunks, args=(timeout, start_time, chunk_queue, shared_best, lock) )
            processes.append(p)
            p.start()
...
def worker_chunks_optimized(timeout, start_time, chunk_queue, shared_best, lock):
    while (time.time() - start_time) < timeout:
        try:
            chunk_start, chunk_end = chunk_queue.get(timeout=0.1)
        except:
        ...
        for number in range(current_start, chunk_end, 2):
            # OPTIMIZATION: Immediate block discard if another worker has surged way ahead
            if chunk_end <= shared_best.value:
                break
...
def find_max_prime_parallel_v4(timeout, workers):
    ...
    with multiprocessing.Manager() as manager:
        ...
        for _ in range(workers):
            p = multiprocessing.Process( target=worker_chunks_optimized, args=(timeout, start_time, chunk_queue, shared_best, lock) )
            processes.append(p)
            p.start()
...
def print_table_row(version_name, prime, execution_time):
    digits = num_digits(prime)
...
if __name__ == "__main__":
    ...
    print_table_header()
    for row in results:
        print_table_row(row[0], row[1], row[2])
    print("=" * 85 + "\n")
